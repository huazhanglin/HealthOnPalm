...
1. **本文依据当前 App 真实界面文案与交互编写**，覆盖注册登录、引导、HealthKit、首页晨报、HOP 助手（含语音）、运动/睡眠记录、档案设置等全部主要流程。  
...
| 图4 | `manual-assets/02-home.png` | 首页与晨报 |
| 图5 | `manual-assets/03-chat.png` | HOP 助手 |
| 图6 | `manual-assets/10-voice-hold.png` | 语音输入 |
| 图7 | `manual-assets/05-records.png` | 健康记录入口 |
| 图8 | `manual-assets/08-workout-log.png` | 记录运动 |
| 图9 | `manual-assets/09-sleep-log.png` | 记录睡眠 |
| 图10 | `manual-assets/06-profile.png` | 我的档案 |
...
| 网络 | 稳定的 Wi‑Fi 或蜂窝网络（登录、晨报、问答、语音均需联网） |
...
拒绝麦克风仍可打字提问；拒绝健康授权仍可使用手动记录与部分模拟数据，但个性化程度会下降。
...
健康数据用于生成**你的个人化建议**；产品定位为个人健康智能体 MVP，**不会把健康数据出售给第三方**。同步到云端是为了生成简报与问答上下文，请仅使用你本人同意用于内测的账号。

---

## 五、首页：晨报、恢复分与今日数据

![首页示意图](./manual-assets/02-home.png)

*图4 首页示意*

首页是每天打开 App 后的主界面。底部有三个主入口：**首页 / 记录 / 我的**。

### 5.1 顶部区域

- 左侧：头像 + **「你好，」** + 你的昵称  
- 右侧：通常显示 **「今日」**；数据加载中可能显示 **「同步中...」**  
...
点击 **「与 HOP 对话」**（或同类入口文案），进入聊天页。

### 5.5 关于「刷新太频繁」的说明

为提升流畅度，首页在短时间内重复进入时**可能不会每次都重新拉数**。若你刚同步了 HealthKit 或刚记了运动/睡眠，可：
...
![记录入口示意图](./manual-assets/05-records.png)

*图7 「记录」Tab 入口示意*

底部 Tab 点击 **「记录」**，进入健康记录枢纽页。
...
页面说明文案大意：记录运动与睡眠，让 HOP 建议更贴合你的状态。
...
3. **运动类型**（必选其一）：跑步、步行、力量训练、瑜伽/拉伸、骑行、HIIT 等。  
...
1. 进入「记录睡眠」。  
2. **日期（起床日）**：选择近 7 天中的起床日（通常不含「今天」作为默认起床日逻辑，以页面可选日期为准）。  
...
5. 在**首页**阅读晨报，点一次「采纳」或「忽略」  
6. 打开 **HOP 助手**，文字问一个睡眠/恢复问题  
7. （可选）**按住语音**说一句话并发送  
8. （可选）对助手回复点 **播放**  
9. 在「记录」里**补一条运动**或**一晚睡眠**  
10. 打开「我的」确认档案无误  
...
完成 HealthKit 授权；打开手机「健康」确认有步数/睡眠；在授权页点「再次同步」；稍后回首页点「刷新」。
...
当前 W3 主路径为 **iPhone + HealthKit**，Android 不在本阶段支持范围。
