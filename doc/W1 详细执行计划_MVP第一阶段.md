...
> **原验收标准（计划稿）**：用户可通过微信小程序完成注册、登录、填写健康档案、查看个人主页  
> **实际验收路径（现行）**：用户可通过 **iOS App（uni-app）邮箱 + 密码** 注册/登录、完成新手引导与健康档案、进入首页  
> **日均投入**：2-4 小时（OPC 模式）  
> **文档状态**：2026-08-24 已按**当前线上/真机运行**修订；下文逐步任务与 SQL 仍保留为当时施工记录，与现行不符处以本节「实际运行对照」为准。

---

## 📋 实际运行对照（2026-08-24）

W1 计划按「微信小程序 + 微信登录 / 手机号 OTP」写。**产品没有走通微信登录，也没有把小程序当主交付物。**

| 计划写法 | 实际运行 |
|----------|----------|
| 主端：微信小程序 | **iOS App**：`uni-app/`（HBuilderX 自定义基座调试，TestFlight 分发） |
| Auth：微信登录 | **未做**；W1 曾规划手机号 OTP，W3 中期改为 **邮箱 + 密码**（现行唯一登录） |
| 代码根：未强调目录 | 移动端在 **`uni-app/src`**；仓库根另有历史 Next.js，**不是**用户打开的 App |
| 后端 | **Supabase** 项目 ref **`zewznptbyhurxaqirzmb`**（新加坡等区域以 Dashboard 为准） |
| 首页 | 有；W1 仅为框架。晨报 / HealthKit / 五 Tab 分别在 W2 / W3 / W4 |
| 微信登录、短信 OTP | **现行产品无**；登录页为邮箱注册/登录，内测建议关闭 Confirm email |

**W1 实际交付（仍有效）：** Supabase 项目与核心表 + RLS；uni-app 工程；登录 / 档案 / 新手引导；Pinia `user` store；基础 UI 组件。

后续文档：W2 `doc/W2 详细执行计划_MVP第二阶段.md` · W3 · W4 `doc/W4 详细执行记录_产品闭环与体验增强.md`
...
|---------|---------|-------|---------|----------|
| W1-T1 | Supabase 项目初始化 | P0 | 2h | ✅ 已完成（项目 `zewznptbyhurxaqirzmb`） |
| W1-T2 | 数据库 Schema 创建 | P0 | 4h | ✅ 已完成（后续 W3/W4 有 ALTER） |
| W1-T3 | RLS 安全策略配置 | P0 | 2h | ✅ 已完成 |
| W1-T4 | Supabase Auth（计划：微信 / 手机号） | P0 | 3h | ♻️ **现行为邮箱密码**（W3 切换） |
| W1-T5 | uni-app 项目初始化 | P0 | 2h | ✅ 已完成（`uni-app/`） |
| W1-T6 | 用户注册/登录页面 | P1 | 4h | ✅ 已完成（邮箱 UI，非手机号） |
| W1-T7 | 用户档案 + 新手引导 | P1 | 4h | ✅ 已完成 |
| W1-T8 | 集成测试 + Bug 修复 | P0 | 4h | ✅ 主路径可用（登录→引导→首页） |
| **总计** | | | **25h** | |
...
### 目标
完成 RLS 安全策略配置 + Supabase Auth（**计划稿为微信/手机号；现行已改为邮箱密码，见文首对照**）
...
**当时 MVP 决策：先用方案 C（手机号验证码），V1.0 再迁移微信登录**  
**实际演进（2026-08）：未上微信登录；W3 起主路径为邮箱 + 密码，手机号 OTP 已退出产品。**
...
**W1 完成情况总结（2026-08-24 按现行补全）：**

| 任务 | 计划时间 | 完成度 | 备注 |
|------|---------|-------|------|
| Supabase 初始化 | 2h | ✅ | 项目 `zewznptbyhurxaqirzmb` |
| 数据库 Schema | 4h | ✅ | 核心表可用；W3/W4 继续扩列 |
| RLS 策略 | 2h | ✅ | 用户只读写自己的数据 |
| Auth 配置 | 3h | ♻️ | 计划微信/手机号 → **现行邮箱密码** |
| uni-app 初始化 | 2h | ✅ | `uni-app/` + HBuilderX |
| 登录页面 | 4h | ✅ | `pages/login/index.vue` 邮箱 |
| 档案页面 | 4h | ✅ | 档案 + onboarding |
| 集成测试 | 4h | ✅ | 真机/TestFlight 主路径可跑 |
| **总计** | **25h** | | |

**遇到的问题：**
1. 计划把微信小程序当验收端，但 HealthKit 无法在小程序验证，主路径改为 iOS App。  
2. 微信登录成本高，手机号 OTP 又有短信成本，最终改为邮箱密码。  

**学到的经验：**
1. 健康数据类 MVP 必须以 iOS 真机 + 自定义基座为准，不能用 H5 代替验收。  
2. 文档里的「微信 / OTP」步骤仅作历史，实施以 `uni-app/src` 为准。  

**W2 规划预览（当时）：** 晨报 Agent、对话、Mock 数据。  
**说明：** HealthKit 同步在 **W3** 完成，不在 W2；每日推送 **至今未做**（用户打开 App 再生成简报）。

---

## 📊 W1 里程碑验收清单

> 勾选状态 = **现行产品**（2026-08-24），不是 2026-07 计划截稿时的空表。

### 必须完成（P0）

- [x] 用户可注册/登录（**现行：邮箱 + 密码**；原「手机号验证码」已废弃）
- [x] 登录后自动创建 / 关联 `public.users` 记录
- [x] 用户可填写和保存健康档案
- [x] 首页显示用户基本信息（昵称等）
- [x] RLS 策略生效（数据隔离）
- [x] 登录 → 引导 → 首页主路径无阻塞性 Bug

### 可选完成（P1）

- [x] 新手引导流程完整
- [x] UI 组件库可用（HaButton / HaCard 等）
- [ ] 性能优化（加载速度 < 2s）— 未作为硬指标度量
- [ ] 错误监控配置（Sentry 等）— 未接入

### 不做（MVP 后处理）

- ❌ 微信登录（计划 V1.0，**至今未做**）
- ❌ 真实健康数据同步（**W3** 实现，不是 W2）
- ❌ AI 功能（**W2–W4**）
- ❌ 订阅付费（至今未做）
